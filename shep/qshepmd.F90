...
!            for loops.
 ...
 integer, parameter :: NPTSSIZE = 1000
 INTEGER, DIMENSION(NPTSSIZE) :: NPTS
...
!$omp parallel do &
!$omp default(shared) &
!$omp private(K,FK,RS,IER,NP,LNP,NPTS,SUM,RWS,RQ) &
!$omp private(NECESSARY,BRK,RSOLD,NEQ,AVSQ,AV) &
!$omp private(NECESSARY4,I,IROW,IRM1,J,IJ,JJ) &
!$omp private(C,S,DMIN,NECESSARY7,NI,NJ,IB,T) &
!$omp copyin(A,IW,DX,XMIN,WS,LCELL,LNEXT)
 DO K=1,N
...
! Test the stabilized system for ill-conditioning.
         ...
         IF (DMIN*RQ <DTOL) THEN
            IER=3
            write(*,*)'DTOL DMIN RQ= ',DTOL,DMIN,RQ
            stop 'IER=3'
         END IF
      END IF
