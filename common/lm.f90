...
contains
  ...
  function decompactify(p,pmin,pmax,ndim)
    ...
    real(lmdp), dimension(ndim) :: decompactify
    real(lmdp), dimension(ndim), intent(in) :: p,pmin,pmax
    real(lmdp), dimension(ndim) :: t
    ...
    t = (p - 0.5_lmdp*(pmin+pmax)) / (0.5_lmdp*(pmax-pmin))

    if (any(abs(t).ge.1._lmdp)) stop 'decompactify: pb!'

    do i=1,ndim
       if (t(i).eq.1._lmdp) t(i)=1._lmdp-epsilon(1._lmdp)
       if (t(i).eq.-1._lmdp) t(i) = -1._lmdp + epsilon(1._lmdp)
    enddo

    decompactify = t/(1._lmdp-abs(t))
  ...
  function compactify(pinf,pmin,pmax,ndim)
    implicit none
    integer, intent(in) :: ndim
    real(lmdp), dimension(ndim) :: compactify
    real(lmdp), dimension(ndim),intent(in) :: pinf,pmin,pmax
    real(lmdp), dimension(ndim) :: t

    t = pinf/(1._lmdp+abs(pinf))

    if (any(abs(t).ge.1._lmdp)) stop 'compactify pb!'

    compactify = 0.5_lmdp*(pmin+pmax) + 0.5_lmdp*(pmax-pmin)*t
